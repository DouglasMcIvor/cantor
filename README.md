...
- **Named union sets** — `Shape = distinct (Circle: Nat | Radius: NatPos)` labels each arm of a `distinct` union, auto-generating one constructor per arm (`Shape.Circle : Nat -> Shape`, `Shape.Radius : NatPos -> Shape`). Each constructor reuses the *same* `mk_Shape`/`from_Shape` uninterpreted-function pair an ordinary single-basis `distinct` set already gets — only the per-arm basis obligation differs (`Shape.Circle(-1)` is rejected with a counterexample the same way a bare `litre(-1)`-style violation would be). `Shape.Circle`/`.Radius` is dotted call syntax resolved structurally at parse/elaboration time (no general namespacing yet — see "on the roadmap" below), reusing the ordinary `Call` path, not a new expression form.
  > **v0 scope:** every arm must share one Kind (not necessarily `Int` any more — `Bool`, a tuple, a vector, etc. are all fine, as long as every arm agrees) — a named union whose arms have *genuinely different* Kinds from each other (`Circle: Nat | Rect: Nat * Nat`) is rejected at compile time with a clear "not yet supported" error, never silently miscompiled, since `Shape` would need to become a real tagged-union-shaped `distinct` (per-arm cross-kind wrapping into/out of `mk_Shape`/`from_Shape`) rather than a single opaque sort. Lifting that restriction (and the destructuring `Tree.leaf2(x, y)`-style patterns that would go with it) is tracked separately. Arms aren't required to be disjoint — same-Kind arms (`Circle: Nat | Weird: Nat`) are allowed but indistinguishable at runtime, since v0 carries no tag.
