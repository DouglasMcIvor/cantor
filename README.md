...
- **`Float32`/`FiniteFloat32`** — IEEE 754 binary32, backed by cvc5's native FloatingPoint theory; literals `3.14f`/`3f`/`1e10f`, `infinity32`/`nan32`/`-infinity32`/`-0.0f`; `+ - * / neg` (no widening — stays `Float32`, unlike `Int`); `FiniteFloat32` excludes `±infinity32`/`nan32` and is a real, provable domain (`require`/`assert` work exactly like any other Kind). `=`/`!=` are SMT-LIB FP equality (reflexive even for `nan32`, `0.0f ≠ -0.0f`) rather than IEEE `==`, matched exactly at runtime (bit-pattern-equal OR both-NaN, not a plain float compare) so nothing the compiler proves about `=` can disagree with what actually runs; ordered comparisons (`< <= > >=`) are real IEEE "false whenever either side is NaN" semantics. No `Float64`, no transcendental functions, and no implicit `Int`↔`Float32` conversion yet — see `docs/design-decisions.md`'s `Float32`/`FiniteFloat32` section.
...
- **Ordered guard groups** — a signature followed directly by 2+ bodies with *no* repeated signature line between them (as opposed to the ordinary overloading form above, which always repeats the signature on every arm) forms an ordered guard group: arms are tried in declaration order, first matching guard wins, and their domains need **not** be pairwise disjoint. In exchange, the compiler proves *coverage* — the union of every arm's (guard-narrowed) domain must equal the group's declared domain; a gap is a compile error with a witness, exactly like an overlap is for the hand-written disjoint form, and an unprovable coverage claim is a compile-time `unknown`, never a silent pass. A new `_` wildcard parameter matches unconditionally and introduces no binder; it's *required* (not just idiomatic) for a catch-all arm inside an ordered group — a bare, unguarded identifier is rejected everywhere in the group (not only the last arm), since it would otherwise be indistinguishable from `_` while silently swallowing every arm declared after it. A trailing all-`_` arm also makes the coverage proof free (no solver call at all): that arm's own domain is definitionally the group's whole declared domain. The two overloading forms are syntactically distinct and can't mix for the same `(name, arity, Kind-bucket)`.
  ```haskell
  sign : Int -> Int
  sign(x for x < 0) = -x
  sign(x for x > 0) = x
  sign(_) = 0
  ```
  > **Known gap:** a call whose candidates belong to an ordered group is never statically resolved to a direct call, even when the argument is a literal the solver could otherwise place in exactly one arm — every such call goes through the runtime dispatch chain instead. The static-resolution shortcut ordinary overloads use isn't sound here unmodified (a trailing wildcard's domain is unconditionally "provable", which doesn't mean it's the first-declared match for every value), and proving it properly needs an extra "no earlier arm can also match" obligation not yet implemented; tracked in backlog.md.
...
- **`distinct` value proofs** — `Litre = distinct Nat` automatically provides the constructor `litre : Nat -> Litre` and the built-in destructor `from(x)` which returns the basis-type value. The solver gives each distinct set its own uninterpreted CVC5 sort plus uninterpreted constructor/destructor functions (`mk_Litre : Int -> Litre`, `from_Litre : Litre -> Int`, or more generally `mk_D : basis_sort -> D`/`from_D : D -> basis_sort` for whatever CVC5 sort the basis has — `distinct` works over `Bool`, `Char`, a tuple, a vector, or a `distinct`-over-`distinct` chain, not just `Int`); basis-set constraints are emitted on demand at each `litre(n)` / `from(x)` site (no global axioms; logic `ALL`); identity functions (`volume : Litre -> Litre`) are proved directly. Plain integer literals not wrapped in a constructor are correctly rejected with a counterexample. Both `litre` and `from` are identity operations at runtime (`distinct` never creates a new runtime Kind, only a new solver-opaque sort). `from` and `size` are reserved keywords.
- **Named union sets** — `Shape = distinct (Circle: Nat | Radius: NatPos)` labels each arm of a `distinct` union, auto-generating one constructor per arm (`Shape.Circle : Nat -> Shape`, `Shape.Radius : NatPos -> Shape`). Labeled arms are always tag-forced — folded via `+` (disjoint union) rather than `|` at parse time, so a label always gets a real runtime tag even when every arm shares one Kind (`Circle: Nat | Radius: NatPos`, both `Kind::Int` — `Shape.Circle(3)` and `Shape.Radius(3)` are provably, and at runtime, distinct values, not the same identity-passthrough integer). Arms may also have genuinely *different* Kinds from each other (`Circle: Nat | Rect: Nat * Nat`) and may freely mix same-Kind and differing-Kind arms in one union (`Circle: Nat | Square: NatPos | Rect: Nat * Nat`) — either way `Shape` is a real cross-kind CVC5 datatype at the solver layer (`mk_Shape`/`from_Shape` connect to that datatype's own sort, wrapping/unwrapping through the matching arm constructor by position, not by Kind) and the `{ i32 tag, i64 leaves }` struct at the LLVM level (built via the same machinery ordinary `A | B` union coercion already uses). The per-arm basis obligation is checked the same way regardless (`Shape.Circle(-1)` is rejected with a counterexample the same way a bare `litre(-1)`-style violation would be). `Shape.Circle`/`.Radius` is dotted call syntax resolved structurally at parse/elaboration time (no general namespacing yet — see "on the roadmap" below), reusing the ordinary `Call` path, not a new expression form.
  > **Known gap:** projecting into a specific arm after `from()` (e.g. `from(Shape.Rect((3,4))).0`) isn't provable yet — a pre-existing cross-kind-union narrowing limitation that also affects plain non-`distinct` unions, tracked in backlog.md; round-trip correctness for heterogeneous arms is instead verified via `show()`, which needs no narrowing. Separately, `!=`/`==` between two `TaggedUnion`-Kind values is unsound at codegen today (drops the tag, compares payload only) — tracked in backlog.md, worked around in the test suite by avoiding `!=`/`==` on these values.
- **Constructor patterns** — `area(Shape.Circle(r)) = r * r` matches a function parameter against one labeled arm of a named union, destructuring that arm's own payload into fresh binder names for the body (a tuple arm destructures element-by-element: `area(Shape.Rect(x, y)) = x * y`). Sugar over the same overloading machinery as guards/literal-arms above: desugars to a domain-narrowing comprehension filter plus a body-prelude `let`/destructuring `let` extracting the payload, reusing the existing comprehension/destructuring machinery end to end with two small new internal-only solver/codegen primitives (a per-arm runtime tag test and payload extractor) — no new syntax at the `SemExprKind` level. Overload disjointness between two constructor-pattern arms of the same function is proved the same way as any other overload set.
  > **v0 scope:** non-recursive named unions only (recursive sets aren't implemented past the well-foundedness check — see "on the roadmap" below). Also limited to call sites where the argument's arm is visible to the solver at the call site (a literal constructor, or anything else the solver can already resolve statically) — a call like `area(pick(b))`, where `pick`'s return value's arm depends on a value not known until runtime and `pick` is checked in a separate solver instance, currently reports `unknown`/counterexample rather than a proof. Closing that gap needs a cross-function invariant that was prototyped as a universally-quantified solver axiom and reverted: quantifying over the union's datatype sort with an arithmetic body made cvc5 report the whole assertion set as inconsistent (both a claim and its negation "provable") — a genuine solver-level dead end, not a quick fix; see backlog.md.
...
- **Recursive named unions** (`Tree = distinct (Leaf: Int | Node: Tree * Tree)`) — constructor patterns (see Features above) work today for non-recursive unions; recursive sets are only implemented through the Phase 0 well-foundedness check, with no solver/codegen support yet, so a self-referential union can't currently be defined at all, let alone pattern-matched over.

- **Named product sets** — `Point = distinct (x: Metre, y: Metre)`; field access via `p.x`. The other half of "namespaces and named structured data" — named union sets, below — is done; products (and general namespacing/field access beyond a named-union constructor) are still on the roadmap. Products have projections, coproducts have injections — the syntax makes the duality explicit.
...
- **More built-in values and collections** — rationals, characters, bytes, maps, `Size`/`Nat64` (a set aliasing the machine word size); bags as `X* / sort` (quotient by permutation) and ordered sets as sets-with-enumerators rather than a new literal bracket — see `docs/design-decisions.md`'s "collections direction".
...
- cvc5 1.3.1, **built with `--poly`** — the exact version the `cvc5` crate's
  C bindings are generated against, and this project relies on cvc5's
  `--nl-cov` option (a libpoly-based nonlinear-arithmetic solver; see
  `docs/design-decisions.md` and `src/solver/mod.rs`), which only exists in
  builds configured with `--poly`. Older packaged versions (e.g. Debian's
  `libcvc5-dev` 1.1.2 on trixie) have a renamed/incompatible C API and will
  fail to build; Ubuntu 26.04's `libcvc5-dev` (1.3.2-1build1) matches the API
  but is built *without* `--poly`, so it builds fine but fails at runtime with
  `Cannot use --nl-cov due to configuring without --poly`. Don't use apt for
  this dependency on any distro — instead install the
  [prebuilt 1.3.1 release](https://github.com/cvc5/cvc5/releases/tag/cvc5-1.3.1)
  (built with `--poly`) under `/usr/local` (see `Dockerfile` and
  `.github/workflows/rust.yml`, which both do this). Since it's installed
  somewhere other than a `libcvc5-dev`-style system path, set `CVC5_LIB_DIR`
  (e.g. `/usr/local/lib`) so `cvc5-sys`'s build script passes it to the
  linker — rustc links with its bundled `rust-lld`, which (unlike GNU `ld`)
  does not search `/usr/local/lib` by default.
