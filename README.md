...
- **Named union sets** — `Shape = distinct (Circle: Nat | Radius: NatPos)` labels each arm of a `distinct` union, auto-generating one constructor per arm (`Shape.Circle : Nat -> Shape`, `Shape.Radius : NatPos -> Shape`). Arms can share one Kind (`Circle: Nat | Radius: NatPos`, both `Kind::Int` — reuses the *same* `mk_Shape`/`from_Shape` uninterpreted-function pair an ordinary single-basis `distinct` set already gets, identity at runtime, no tag) or have genuinely *different* Kinds from each other (`Circle: Nat | Rect: Nat * Nat`) — in that case `Shape` becomes a real tagged union: a cross-kind CVC5 datatype at the solver layer (`mk_Shape`/`from_Shape` now connect to that datatype's own sort, wrapping/unwrapping through the matching arm constructor) and the `{ i32 tag, i64 leaves }` struct at the LLVM level (built via the same machinery ordinary `A | B` union coercion already uses). Either way, the per-arm basis obligation is checked the same way (`Shape.Circle(-1)` is rejected with a counterexample the same way a bare `litre(-1)`-style violation would be). `Shape.Circle`/`.Radius` is dotted call syntax resolved structurally at parse/elaboration time (no general namespacing yet — see "on the roadmap" below), reusing the ordinary `Call` path, not a new expression form.
  > **v0 scope:** every arm's Kind must be pairwise distinct from every *other* arm's Kind whenever the arms aren't all the same Kind — `Circle: Nat | Square: NatPos | Rect: Nat * Nat` (two `Int`-Kind arms mixed with a `Tuple`-Kind one) is rejected at compile time with a clear "not yet supported" error, never silently miscompiled. This sidesteps a suspected pre-existing gap in the general cross-kind union machinery (constructor names are derived purely from Kind, with no disambiguation when two arms share one — tracked in backlog.md, not specific to named unions). Projecting into a specific arm after `from()` (e.g. `from(Shape.Rect((3,4))).0`) isn't provable yet either — a separate, pre-existing cross-kind-union narrowing limitation that also affects plain non-`distinct` unions, tracked alongside the constructor-pattern work in backlog.md; round-trip correctness for heterogeneous arms is verified via `show()` instead, which needs no narrowing.
