...
fn parse_error_message_goes_to_stderr() {
    ...
    assert!(
        !out.stderr.is_empty(),
        "expected parse error on stderr, stdout was: {:?}",
        out.stdout
    );
}
...
fn good_file_shows_signatures() {
    ...
    let out = run_file("good.cantor");
    assert!(
        out.stdout.contains("Int -> Nat"),
        "abs sig missing:\n{}",
        out.stdout
    );
    assert!(
        out.stdout.contains("Nat -> Nat"),
        "double/quad sig missing:\n{}",
        out.stdout
    );
}
...
fn bad_file_exits_nonzero() {
    ...
    assert_ne!(
        out.code, 0,
        "bad.cantor should exit non-zero\nstdout: {}",
        out.stdout
    );
}
...
fn counterexample_output_shows_witness_and_range() {
    ...
    assert!(
        out.stdout.contains("Int16"),
        "expected Int16 range:\n{}",
        out.stdout
    );
    assert!(
        out.stdout.contains("Nat"),
        "expected Nat range:\n{}",
        out.stdout
    );
}
