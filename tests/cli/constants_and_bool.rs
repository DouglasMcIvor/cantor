...
fn bool_demo_all_proved() {
    ...
    assert_eq!(
        out.code, 0,
        "bool_demo.cantor should exit 0\nstdout: {}",
        out.stdout
    );
    assert!(
...
fn bool_demo_shows_bool_signatures() {
    let out = run_file("bool_demo.cantor");
    assert!(
        out.stdout.contains("Int -> Bool"),
        "expected 'Int -> Bool' sig:\n{}",
        out.stdout
    );
    assert!(
        out.stdout.contains("Bool -> Bool"),
        "expected 'Bool -> Bool' sig:\n{}",
        out.stdout
    );
    assert!(
        out.stdout.contains("Bool -> Nat"),
        "expected 'Bool -> Nat' sig:\n{}",
        out.stdout
    );
}
