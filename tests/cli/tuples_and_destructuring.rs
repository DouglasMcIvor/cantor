...
fn tuple_basics_all_proved() {
    ...
    assert_eq!(
        out.code, 0,
        "tuple_basics.cantor should exit 0\nstdout: {}",
        out.stdout
    );
    assert!(
...
fn tuple_basics_shows_product_signatures() {
    let out = run_file("tuple_basics.cantor");
    assert!(
        out.stdout.contains("Int * Int -> Int"),
        "fst/snd sig missing:\n{}",
        out.stdout
    );
    assert!(
        out.stdout.contains("Nat * Nat -> Nat"),
        "sum_pair sig missing:\n{}",
        out.stdout
    );
    assert!(
        out.stdout.contains("Int * Int -> Int * Int"),
        "swap/identity sig missing:\n{}",
        out.stdout
    );
}
...
fn tuple_bad_counterexample() {
    ...
    assert_ne!(
        out.code, 0,
        "tuple_bad.cantor should exit non-zero\nstdout: {}",
        out.stdout
    );
    assert!(
        out.stdout.contains("  counterexample  "),
...
fn destructure_basics_all_proved() {
    ...
    assert_eq!(
        out.code, 0,
        "destructure_basics.cantor should exit 0\nstdout: {}",
        out.stdout
    );
    assert!(
...
fn destructure_basics_run_produces_correct_output() {
    ...
    assert_eq!(
        out.code, 0,
        "destructure_basics.cantor run should exit 0\nstdout: {}",
        out.stdout
    );
    // main() returns -3 + 4 = 1
    assert!(
...
fn destructure_bad_gives_counterexample() {
    ...
    assert_ne!(
        out.code, 0,
        "destructure_bad.cantor should exit non-zero\nstdout: {}",
        out.stdout
    );
    assert!(
        out.stdout.contains("  counterexample  "),
...
fn destructure_mut_run_produces_correct_output() {
    ...
    assert_eq!(
        out.code, 0,
        "destructure_mut.cantor run should exit 0\nstdout: {}",
        out.stdout
    );
...
fn destructure_partial_all_proved() {
    ...
    assert_eq!(
        out.code, 0,
        "destructure_partial.cantor should exit 0\nstdout: {}",
        out.stdout
    );
    assert!(
        !out.stdout.contains("  counterexample  ") && !out.stdout.contains("  unknown  "),
...
fn destructure_partial_run_produces_correct_output() {
    ...
    assert_eq!(
        out.code, 0,
        "destructure_partial.cantor run should exit 0\nstdout: {}",
        out.stdout
    );
...
fn destructure_local_tuple_var_all_proved() {
    ...
    assert_eq!(
        out.code, 0,
        "destructure_local_tuple_var.cantor should exit 0\nstdout: {}",
        out.stdout
    );
    assert!(
        !out.stdout.contains("  counterexample  ") && !out.stdout.contains("  unknown  "),
...
fn destructure_local_tuple_var_run_produces_correct_output() {
    ...
    assert_eq!(
        out.code, 0,
        "destructure_local_tuple_var.cantor run should exit 0\nstdout: {}",
        out.stdout
    );
...
fn vector_destructure_let_clean_error() {
    ...
    assert_ne!(
        out.code, 0,
        "vector_destructure_let.cantor should exit non-zero:\n{}",
        out.stdout
    );
    assert!(
...
fn vector_param_destructure_clean_error() {
    ...
    assert_ne!(
        out.code, 0,
        "vector_param_destructure.cantor should exit non-zero:\n{}",
        out.stdout
    );
    assert!(
