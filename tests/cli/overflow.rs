...
fn unbounded_mul_aborts_at_runtime_not_a_wrong_value() {
    ...
    assert_ne!(out.code, 0, "overflow_mul.cantor should abort at runtime:\nstdout: {}\nstderr: {}", out.stdout, out.stderr);
    assert!(
...
fn unbounded_mul_runs_normally_when_no_overflow_occurs() {
    ...
    assert!(out.stdout.contains("main() = 42"), "expected correct result:\n{}", out.stdout);
}

#[test]
fn unbounded_add_aborts_at_i64_max() {
    let out = run_subcommand("overflow_add.cantor");
    assert_ne!(out.code, 0, "overflow_add.cantor should abort at runtime:\n{}", out.stdout);
    assert!(out.stderr.contains("arithmetic overflow"), "stderr: {}", out.stderr);
}

#[test]
fn unbounded_sub_aborts() {
    let out = run_subcommand("overflow_sub.cantor");
    assert_ne!(out.code, 0, "overflow_sub.cantor should abort at runtime:\n{}", out.stdout);
    assert!(out.stderr.contains("arithmetic overflow"), "stderr: {}", out.stderr);
}
...
fn negating_i64_min_aborts() {
    ...
    assert_ne!(out.code, 0, "overflow_neg.cantor should abort at runtime:\n{}", out.stdout);
    assert!(out.stderr.contains("arithmetic overflow"), "stderr: {}", out.stderr);
}
...
fn division_of_i64_min_by_neg_one_aborts() {
    ...
    assert_ne!(out.code, 0, "overflow_div_min_neg1.cantor should abort at runtime:\n{}", out.stdout);
    assert!(out.stderr.contains("arithmetic overflow"), "stderr: {}", out.stderr);
}
...
fn ordinary_division_unaffected_by_overflow_channel() {
    ...
    assert_eq!(out.code, 0, "expected exit 0:\nstdout: {}\nstderr: {}", out.stdout, out.stderr);
    assert!(out.stdout.contains("main() = 3"), "expected correct result:\n{}", out.stdout);
}
