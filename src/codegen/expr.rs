...
impl<'ctx> Compiler<'ctx> {
    ...
    fn compile_call(
        ...
        let (lookup_key, target): (String, CallTarget<'_>) =
            match self.overload_dispatch.get(&callee.0) {
                None => (callee.0.clone(), CallTarget::Direct(callee.0.clone())),
                Some(entries) => {
                    ...
                    if let [only] = matching.as_slice() {
                        ...
                        // all of them, used for arg coercion below — `matching[0]`
                        // is as good as any (see `OverloadEntry`'s doc comment
                        // for the one place this representative choice is
                        // imprecise: a candidate with more than one signature of
                        // its own).
                        (
