...
- Compiler must prove function composition respects domains/ranges (e.g.
  ...
  proof gap, generated by **abduction** (cvc5 `get-abduct`). Not the unsat
  core: a failed check returns `sat`, so there is no core to read — the core
  only exists when the proof *succeeded*, where it powers a separate
  over-constraint diagnostic instead. See backlog.md's "error messages".
